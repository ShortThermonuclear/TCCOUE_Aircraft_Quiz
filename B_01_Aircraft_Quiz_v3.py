...
class StartQuiz:
    ...
    def __init__(self):
        ...
        # Keeps track of whether the Help window is currently open,
        # so that to_quiz() can close it if the user starts a quiz.
        ...
        # Exit button - replaced the overall stats button.
        self.exit_button = Button(self.bottom_btn_frame, text="Exit Quiz",
                                   font=["Helvetica", "16"],
                                   fg="#330000", bg="#D3D3D3", width=15, height=1,
                                  command=root.destroy) # completely closes the whole window
        self.exit_button.grid(row=0, column=1, padx=25, pady=10)
        ...
        self.help_button.grid(row=0, column=0, padx=25, pady=10)
    ...
    def to_help(self):
        ...
        """
        # Stores the help window so to_quiz() can close it if needed.
        self.help_window = DisplayHelp(self)
        self.help_button.config(state="disabled")


    def to_quiz(self, quiz_type):
        """Launches Play with the chosen difficulty / quiz_type."""
        # Closes the help window if it's still open when a
        # difficulty button is pressed.
        if self.help_window is not None and self.help_window.help_box.winfo_exists():
            self.help_window.help_box.destroy()
...
class DisplayHelp:
    ...
    def __init__(self, partner):
        ...
        hard_text = ("Choose the correct name from 4 options. "
                     "You only have 20 seconds per question!")
        ...
        for count, item in enumerate(help_labels_list):
            # Tells the editor that item[3] is always a valid justify value.
            label_justify = cast(Literal["left", "center", "right"], item[3])
            make_label = Label(self.help_frame, text=item[0], font=item[1],
                               fg=item[2], justify=label_justify,
    ...
    def close_help(self, partner):
        ...
        # Sets this back to None now the window is closed, so
        # to_quiz() knows there's nothing left to close.
        partner.help_window = None
...
class PlayQuiz:
    ...
    def __init__(self, how_many, quiz_type):
        ...
        self.play_box.resizable(False, False)  # Stops the window being resized.
        ...
        for item in range(btn_count):
            answer_button = Button(self.answer_frame, text=self.answer_options[item],
                         ...
                         width=18, padx=8, pady=4, wraplength=140,
    ...
    def new_question(self):
        ...
        # print statement to get correct ans
    ...
    def check_answer(self, answer_index):
        ...
        aircraft_name = self.current_aircraft[0]

        # Add one to the number of questions played
        self.questions_played.set(self.questions_played.get() + 1)

        # maps each of the quiz types to feedback message
        # so each difficulty chosen will have their own feedback messages.
        feedback_map = {
            "country": f"The {aircraft_name} was made in {self.correct_answer}.",
            "purpose": f"The {aircraft_name} is used for {self.correct_answer}.",
            "name": f"That aircraft is the {self.correct_answer}.",
        }
        feedback_detail = feedback_map.get(self.quiz_type)

        # Check if answer is correct and edits the feedback label
        if chosen_answer == self.correct_answer:
            self.feedback_label.config(text=f"✓ Correct! {feedback_detail}", fg="green")
            self.questions_correct.set(self.questions_correct.get() + 1)
            self.all_results_list.append(True) # stores this as correct

        else:
            self.feedback_label.config(
                text=f"✗ Incorrect — {feedback_detail}",
                fg="red"
            )
    ...
    def handle_game_over(self):
        """Updates the UI when all questions have been answered."""
        self.cancel_timer()
        if self.quiz_type == "name":
            self.timer_label.config(text="")

        # Calculates the percentage for the final score
        correct = self.questions_correct.get()
        total = self.questions_played.get()
        pct = correct / total * 100

        # configures labels to show that the quiz has ended +
        # with percentage score.
        # configures end game button to be "play again"
        self.num_question_label.config(text="Quiz Complete!")
        self.question_label.config(
            text=f"You got {correct} of {total} correct ({pct:.0f}%).")
        self.btn_next.config(state="disabled", text="Done")
        self.btn_end.config(text="Play Again", bg="#A8D5A2",
                            command=self.close_play
                            )
    def to_hints(self):
        """Opens hints window."""
        # Disables End, Hints and Stats while Hints is open, so the user
        # can't end the quiz or open Stats while Hints is still showing.
        self.btn_end.config(state="disabled")
        self.btn_hints.config(state="disabled")
        self.btn_stats.config(state="disabled")
        DisplayHints(self, self.current_aircraft, self.quiz_type)

    def to_stats(self):
        """Opens stats window."""
        # Disables End, Hints and Stats while Stats is open, so the user
        # can't end the quiz or open Hints while Stats is still showing.
        self.btn_end.config(state="disabled")
        self.btn_hints.config(state="disabled")
        self.btn_stats.config(state="disabled")
        DisplayStats(self)
    ...
    def close_play(self):
        ...
        # quiz - allows new quiz to start. Good for testing as well.
        root.deiconify()
...
class DisplayHints:
    ...
    def __init__(self, partner, aircraft, quiz_type):
        ...
        self.hint_box.resizable(False, False)
    ...
    def close_hints(self, partner):
        """Closes hints window and re-enables the nav bar buttons again."""

        partner.btn_hints.config(state="normal")
        partner.btn_end.config(state="normal")

        # Only re-enable Stats if at least one question has been
        # answered, otherwise there's nothing to show yet.
        if partner.all_results_list:
            partner.btn_stats.config(state="normal")

        # destroy the hints window
...
class DisplayStats:
    ...
    def close_stats(self, partner):
        """Closes stats window and re-enables the nav bar buttons again."""

        # enables all other navigator buttons
        partner.btn_stats.config(state="normal")
        partner.btn_hints.config(state="normal")
        partner.btn_end.config(state="normal")

        # destroys the stats window
